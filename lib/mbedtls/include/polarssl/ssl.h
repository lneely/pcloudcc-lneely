...
#define POLARSSL_ERR_SSL_NO_USABLE_CIPHERSUITE                                 \
  -0x6A80 /**< None of the common ciphersuites is usable (eg, no suitable      \
...
#define SSL_BUFFER_LEN                                                         \
  (SSL_MAX_CONTENT_LEN + SSL_COMPRESSION_ADD + 29 /* counter + header + IV */  \
   + SSL_MAC_ADD + SSL_PADDING_ADD)
...
struct _ssl_context {
  ...
  char own_verify_data[SSL_VERIFY_DATA_MAX_LEN]; /*!<  previous handshake verify
                                                    data */
  char peer_verify_data[SSL_VERIFY_DATA_MAX_LEN]; /*!<  previous handshake
                                                     verify data */
#endif
};
...
extern int (*ssl_hw_record_init)(ssl_context *ssl, const unsigned char *key_enc,
 ...
 */
void ssl_set_dbg(ssl_context *ssl, void (*f_dbg)(void *, int, const char *),
                 void *p_dbg);
 ...
 */
void ssl_set_ca_chain(ssl_context *ssl, x509_crt *ca_chain, x509_crl *ca_crl,
                      const char *peer_cn);
 ...
 * \return          0 on success, or a specific error code.
 */
int ssl_set_own_cert_alt(ssl_context *ssl, x509_crt *own_cert, void *rsa_key,
                         rsa_decrypt_func rsa_decrypt, rsa_sign_func rsa_sign,
 ...
 */
int ssl_send_alert_message(ssl_context *ssl, unsigned char level,
